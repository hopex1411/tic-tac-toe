...
function Board (options){
  ...
  function initGame(){
    state.game= {
      ...
      turn: Math.round(Math.random()), // This var is set randomly for the first move.
    }
  }

  function moveCount(board){
    // Receives a board and returns the number of played moves.
    let moveCount = 0
  ...
  function getResult(board,symbol){
      // Receives a board and the player symbol, and returns an object with the result and an array of the winning line.
  ...
  function render(){
    ...
    function htmlQ1(){
      return `<div id="view1"><h3>Choose gamemode\n</h3>
      ${buttonHTML(1, "1player", "Play against the computer")}
      ${buttonHTML(1, "2players", "Play with a friend")}
      </div>`
    }
  ...
  function executeTurn(board, move, symbol) {
    ...
    if (result === RESULT.incomplete){
      ...
      // Increment score and shows result.
      if(result !== RESULT.tie) {
